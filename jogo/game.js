  ...
  function spellUI(q) {
    ...
    function draw() {
      ...
      target.split('').forEach(function (ch) {
        ...
        const idx = li;
        if (v) s.onclick = function () { filled[idx] = null; usedFix(); draw(); };
        slots.appendChild(s);
        li++;
      });
    }
    ...
    function check() {
      ...
      const want = letters.join('');
      const good = got === want;
      Array.prototype.forEach.call(slots.children, function (s) {
        if (!s.classList.contains('space')) s.classList.add(good ? 'ok' : 'bad');
      });
      setTimeout(function () {
        answer(q, good, null, null, null, function () {
          if (!good) { filled.length = 0; usedFix(); draw(); }
        });
      }, good ? 180 : 420);
    }
    ...
    hint.appendChild(hb); hint.appendChild(sb);
