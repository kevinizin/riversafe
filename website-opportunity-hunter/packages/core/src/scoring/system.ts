...
const MATURITY_BANDS: { maxDays: number; points: number; label: string }[] = [
  ...
  { maxDays: 5 * YEAR_DAYS, points: 20, label: '2 a 5 anos — operação consolidada com processo ainda improvisado' },
  { maxDays: 10 * YEAR_DAYS, points: 18, label: '5 a 10 anos — volume alto, e processo manual já sai caro' },
];
...
function scoreSectorFit(
  ...
  if (!input.industryKey) {
    ...
    gaps.push('Setor não identificado, então o encaixe com um sistema não pôde ser avaliado');
    return { component: mk(0, 'Setor não identificado'), useCases: [] };
  }

  const profile = getIndustry(input.industryKey);
  if (!profile) {
    gaps.push(`O setor "${input.industryKey}" não está no catálogo`);
    return { component: mk(0, `Setor "${input.industryKey}" desconhecido`), useCases: [] };
  }

  const points = Math.round(profile.systemWeight * max);
  const strength =
    profile.systemWeight >= 0.85 ? 'depende fortemente de' : profile.systemWeight >= 0.6 ? 'se beneficia de' : 'pouco precisa de';
  return {
    component: mk(points, `${profile.label}: setor que ${strength} um sistema de gestão`),
...
function scoreSizeFit(
  ...
  const window = `${target.min} a ${target.max} pessoas`;
  const mk = (points: number, reason: string): SystemScoreComponent => ({
    component: 'SIZE_FIT',
    points,
    max,
  ...
  switch (fit) {
    ...
    case 'POSSIBLE':
      ...
      return {
        component: mk(
          20,
          `Porte estimado pode estar na faixa alvo (${window}), mas a faixa do próprio registro é mais larga que isso`,
        ),
        fit,
      };
    case 'UNLIKELY':
...
function scoreSystemGap(input: SystemScoreInput, gaps: string[]): SystemScoreComponent {
  ...
  gaps.push('Nenhuma fonte pública diz se a empresa já usa um sistema internamente — confirmar na conversa');

  if (input.noWebsiteFound) {
    return mk(
      max,
      'Sem site nenhum: nenhum canal digital próprio, o que costuma vir junto com papel ou planilha (a confirmar)',
    );
  }

  if (!input.websiteAnalysed) {
    return mk(0, 'Site não analisado — nada observado sobre as ferramentas digitais');
  }
  ...
  if (hasCustomerSystem) {
    return mk(
      3,
      'O site já oferece agendamento on-line, então parte da operação está digitalizada (o que roda por trás não foi confirmado)',
    );
  }

  return mk(
    12,
    'O site é só institucional, sem área do cliente nem agendamento — indício fraco de processo manual (a confirmar)',
  );
}
...
function signalLabel(type: string): string {
  switch (type) {
    case 'HIRING':
      return 'contratando (a equipe está crescendo)';
    case 'NEW_LOCATION':
      return 'abriu uma nova unidade';
    case 'RECENT_REVIEWS':
